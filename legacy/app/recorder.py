...
def pcm16_from_float32(frame: np.ndarray) -> bytes:
    """16 kHz mono frames are float32 in the callback and int16 little-endian on the wire."""
...
class Recorder:
    ...
    def set_pcm_sink(self, sink) -> None:
        """Receive each PCM frame. None clears the sink. The WAV frames stay either way."""
        with self._lock:
            self._pcm_sink = sink
    ...
    def _callback(self, indata, frame_count, time_info, status):
        ...
        if sink is not None:
            try:
                sink(pcm16_from_float32(frame))
            except Exception:
                logger.exception("Live PCM write failed")
