...
from app.transcriber import TranscriptionProvider, create_transcriber, skips_llm_cleanup
...
class Pipeline:
    ...
    def _set_pcm_sink(self, sink) -> None:
        set_sink = getattr(self.recorder, "set_pcm_sink", None)
        if set_sink is not None:
            set_sink(sink)

    def _open_live_session(self) -> None:
        """Gemini Live and Mistral Realtime open a socket here. Groq stays on the WAV."""
        self._live_session = None
        opener = getattr(self.transcriber, "open_live_session", None)
        if opener is None:
            return
        try:
            self._live_session = opener()
        except Exception:
            logger.exception("Live session did not open")
            self._live_session = None
            return
        if self._live_session is not None:
            self._set_pcm_sink(self._live_session.write_pcm)

    def _take_live_session(self):
        session = self._live_session
        self._live_session = None
        self._set_pcm_sink(None)
        return session

    def _close_live_session(self, session) -> None:
        if session is None:
            return
        try:
            session.close()
        except Exception:
            logger.exception("Failed to close live session")

    def _drop_live_session(self) -> None:
        self._close_live_session(self._take_live_session())

    def _transcribe_audio(self, wav_bytes: bytes, live_session):
        if live_session is not None:
            try:
                live_result = live_session.finish()
            except Exception as exc:
                logger.warning("Live transcription failed: %s", exc)
            else:
                if str(getattr(live_result, "text", "") or "").strip():
                    logger.info("Live transcript used")
                    return live_result
                logger.info("Live transcript empty")
            logger.info("Batch transcription fallback")
    ...
    def stop_recording_and_process(
        ...
        *,
        ...
        with self._state_lock:
            ...
            recording_id = self._active_recording_id
            live_session = self._take_live_session()
            t_total = time.perf_counter()
            t_stop = time.perf_counter()
            try:
                wav_bytes = self.recorder.stop(abort=abort_recording_stop)
            except Exception:
                self._close_live_session(live_session)
    ...
    def _process_audio(
        ...
        *,
        ...
        try:
            result = self._transcribe_audio(wav_bytes, live_session)
        except Exception:
...
if __name__ == "__main__":
    ...
    if cfg.transcription.provider == "groq" and not cfg.groq_api_key:
        ...
        sys.exit(1)
    if cfg.transcription.provider == "gemini" and not cfg.gemini_api_key:
        print("Error: GEMINI_API_KEY not set")
        sys.exit(1)
    if cfg.transcription.provider == "mistral" and not cfg.mistral_api_key:
    ...
    skip_cleanup = skips_llm_cleanup(cfg.transcription.provider, cfg.transcription.mode)
    cleanup = create_cleanup(
        ...
        enabled=cfg.cleanup.enabled and not skip_cleanup,
    )
