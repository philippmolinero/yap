...
from app.config import CONFIG_DIR, VOCAB_FILE, load_config
...
from app.transcriber import UnconfiguredTranscriber, create_transcriber, skips_llm_cleanup
...
class YapApp(rumps.App):
    ...
    def _build_pipeline(self):
        ...
        transcriber = create_transcriber(
            ...
            mode=self.cfg.transcription.mode,
        ...
        skip_cleanup = skips_llm_cleanup(
            self.cfg.transcription.provider, self.cfg.transcription.mode
        )
        if skip_cleanup:
            logger.info("Gemini SMART transcription is on; skipping LLM cleanup")
        cleanup_key = {
        ...
        cleanup = create_cleanup(
            ...
            enabled=self.cfg.cleanup.enabled and not skip_cleanup,
        )
    ...
    def _start_hotkey_once(self, timer):
        ...
        missing_cleanup_key = (
            ...
            and not skips_llm_cleanup(
                self.cfg.transcription.provider, self.cfg.transcription.mode
            )
            and (
                (
                    self.cfg.cleanup.provider == "groq"
                    and not self.cfg.groq_api_key
                )
                or (
                    self.cfg.cleanup.provider == "mistral"
                    and not self.cfg.mistral_api_key
                )
                or (
                    self.cfg.cleanup.provider == "cerebras"
